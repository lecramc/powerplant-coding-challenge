...
Plan = TypedDict("Plan", {"name": str, "p": float})
...
class ProductionPlan:
        ...
        for plant in self.plants:
            if isinstance(plant, GasFiredPlant):
                plant.cost = plant.calculate_cost_per_mwh(
                    fuel_prices["gas"], fuel_prices["co2"]
                )
                continue
    ...
    def _allocate_production(self, load: float, wind_percentage: float) -> List[Plan]:
        """Allocates production to meet the requested workload, respecting the order of merit"""
        production_plan: List[Plan] = []
        ...
        for plant in self.plants:
            ...
            production = self._calculate_plant_production(
                plant, load - total_production, wind_percentage
            )
    ...
    @staticmethod
    def _calculate_plant_production(
        plant: PowerPlant, remaining_load: float, wind_percentage: float
    ) -> float:
    ...
    def _include_non_active_plants(self, production_plan: List[Plan]) -> None:
